...
# Хранилище индексов фраз для каждого пользователя
phrase_indexes = {}

def get_next_phrase(user_id, phrase_type):
    """Возвращает следующую фразу по очереди (по кругу)"""
    key = f"{user_id}_{phrase_type}"
    if key not in phrase_indexes:
        phrase_indexes[key] = 0
    
    phrases = PHRASES.get(phrase_type, [])
    if not phrases:
        return ""
...
def add_user(message):
    ...
    try:
        ...
        bot.reply_to(message, "❌ Ошибка. Формат: /adduser 123456789")
...
def process_savings_add(message):
    ...
    try:
        ...
        phrase = get_next_phrase(user_id, "saving").format(amount=int(amount))
        ...
        for s in data["savings"]:
            if name.lower() in s["name"].lower():
                if s["amount"] >= amount:
                    ...
                    phrase = get_next_phrase(user_id, "expense").format(amount=int(amount))
...
def button_budget(message):
    ...
    if data["budget"] and data["budget"]["month"] == current_month and data["budget"]["year"] == current_year:
        ...
        if percent >= 100:
            phrase = get_next_phrase(user_id, "budget_over").format(overflow=int(expenses - budget_amount))
            text += f"\n{phrase}"
        elif percent >= 80:
...
def set_budget(message):
    ...
    try:
        ...
        phrase = get_next_phrase(user_id, "budget_set").format(amount=int(amount))
        bot.reply_to(message, phrase)
    except:
...
def button_spending_chart(message):
    ...
    if total_expenses == 0:
        bot.reply_to(message, f"📊 За {month_name} расходов нет")
        return
...
def button_undo(message):
    ...
    phrase = get_next_phrase(user_id, "undo")
...
def handle_currency_callback(call):
    ...
    if call.data.startswith("income"):
        ...
        phrase = get_next_phrase(user_id, "income").format(amount=int(amount) if currency == "₽" else amount)
        ...
        if need_category:
            ...
            bot.edit_message_text("📂 Выбери категорию расхода:", call.message.chat.id, call.message.message_id, reply_markup=get_category_keyboard("expense"))
        else:
            if currency == "$":
                data["usd"] -= amount
                amount_display = f"{amount:.2f}{currency}"
            ...
            phrase = get_next_phrase(user_id, "expense").format(amount=int(amount) if currency == "₽" else amount)
            bot.edit_message_text(
                f"{phrase}\n🕐 {now.strftime('%H:%M')} МСК",
                call.message.chat.id,
                call.message.message_id
...
def send_monthly_report_to_user(user_id, month, year):
    ...
    phrase = get_next_phrase(user_id, "monthly_report").format(month=month_name, total=int(rub_out))
